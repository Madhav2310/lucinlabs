...
chk "P0-5 --list-rules works"  'lucin scan --list-rules >/dev/null 2>&1'
chk "P0-5 --list-adapters"     'lucin scan --list-adapters >/dev/null 2>&1'
chk "P0-6 real numbers in HTML" 'grep -q "data-target=\"76\"" site/index.html'
chk "P0-7 no stale taxonomy"   '[ "$(lucin scan examples/ --no-telemetry 2>&1 | grep -cE "Cryptographic Failures|Excessive Agency")" = "0" ]'
chk "P0-7 mapping test"        'python -m pytest tests/test_owasp_mapping.py -q >/dev/null 2>&1'
...
chk "P0-9 bars proportional"   'lucin scan examples/vulnerable-agent/ --no-telemetry 2>&1 | grep -A3 "RISK SUMMARY" | grep -q "MEDIUM.*████████████████████████"'
...
chk "P0-10 all cmds opt out"   'for c in scan info explain fix badge discover redteam monitor serve; do lucin $c --help 2>&1 | grep -q -- "--no-telemetry" || exit 1; done'
chk "P0-10 json not polluted"  'lucin scan examples/vulnerable-agent/ --format json 2>/dev/null | python -c "import json,sys; json.load(sys.stdin)"'
chk "P0-11 fix hints runnable" 'python -m pytest tests/test_printed_commands.py -q >/dev/null 2>&1'
chk "P0-11 no --tool in hints" '! lucin scan examples/vulnerable-agent/ --no-telemetry 2>&1 | grep -q -- "fix .* --tool"'
chk "P0-12 baseline flags"     'lucin scan --help 2>&1 | grep -q -- "--write-baseline"'
chk "artifact is clean"        'python -m build --wheel --outdir /tmp/gate_whl >/dev/null 2>&1 && rm -rf /tmp/gate_x && mkdir -p /tmp/gate_x && unzip -qo /tmp/gate_whl/*.whl -d /tmp/gate_x && [ "$(grep -rniE "\bvisa\b|madhmitt|_VISA_CA" /tmp/gate_x | grep -viE "Mastercard|Amex|ending 4242" | wc -l | tr -d " ")" = "0" ]'
chk "tests green"              'python -m pytest tests/ -q >/dev/null 2>&1'
